...
function TelegramCta({ href, title, body, cta }: { href: string; title: string; body: string; cta: string }) {
  return (
    <div className="rounded-lg p-5 mb-8 border text-left" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)' }}>
      <p className="font-medium mb-2">{title}</p>
      <p className="text-sm mb-4" style={{ color: 'var(--muted-fg)' }}>{body}</p>
      <a
        href={href}
...
async function OrderVerifier({ sessionId }: { sessionId: string }) {
  ...
  try {
    const res = await fetch(
      `${process.env.STOREFRONT_URL || 'https://thefoolishbutcher.com'}/api/stripe/session?session_id=${encodeURIComponent(sessionId)}`,
      { cache: 'no-store' },
    )
    if (res.ok) {
  ...
  const telegramHref = orderRef
    ? `https://t.me/the_foolish_butcher_bot?start=order_${orderRef}`
    : 'https://t.me/the_foolish_butcher_bot'

  if (!verified) {
    return (
      <>
        <div className="rounded-lg p-4 mb-6 border text-sm" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)' }}>
          <p style={{ color: 'var(--muted-fg)' }}>{t('subtitle')}</p>
        </div>
        <TelegramCta href={telegramHref} title={t('telegramTitle')} body={t('telegramBody')} cta={t('telegramCta')} />
      </>
    )
  }
