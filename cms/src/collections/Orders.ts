    ...
    // ── Campi tecnici (nascosti) ──────────────────────────────────
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: 'text',
          label: 'Snapshot codice promo affiliato',
          validate: (value: unknown) => value == null || /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(value as string) || 'Il codice promo affiliato non è valido',
        },
        {
          name: 'affiliateSlug',
          type: 'text',
          label: 'Snapshot slug affiliato',
          validate: (value: unknown) => value == null || /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(value as string) || 'Lo slug affiliato non è valido',
        },
        {
          name: 'affiliateEligibleAmountCents',
          type: 'number',
          label: 'Importo affiliato eleggibile (centesimi)',
          validate: (value: unknown) => value == null || (typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= Number.MAX_SAFE_INTEGER) || 'L’importo affiliato deve essere un intero positivo in centesimi',
        },
        {
          name: 'affiliateCommissionRateBps',
          type: 'number',
          label: 'Aliquota affiliato (bps)',
          validate: (value: unknown) => value == null || (typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= 10000) || 'L’aliquota affiliato deve essere un intero tra 0 e 10000 bps',
        },
        {
          name: 'affiliateCommissionCents',
          type: 'number',
          label: 'Commissione affiliato (centesimi)',
          validate: (value: unknown) => value == null || (typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= Number.MAX_SAFE_INTEGER) || 'La commissione affiliato deve essere un intero positivo in centesimi',
        },
